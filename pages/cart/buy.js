...
Page({
  ...
  submitOrder: function (event) {
    ...
    console.log('断点1');
    //if (!this.checkAddress()) return false;  //先去掉 2017年9月5日18:42:35
    console.log('断点2');
    // 收货地址
    //let shippingMethod ='flat.flat'; //2017年9月6日11:12:41 写死测试
    //console.log('收货地址 shippingMethod值是：' + shippingMethod);
   ...
   */
  buildAddressParams() {
    console.log('组装地址参数d1');
   
    let { addressId, zoneId, cityId, districtId, fullname, shippingTelephone, location, shippingMethod, pickupStoreId } = this.data;
    let params;
    console.log('shippingMethod值是：' + shippingMethod);
    if (shippingMethod == 'flat.flat') {
      console.log('组装地址参数d2');
      if (addressId) {
        console.log('组装地址参数d3');
        params = {
          'addressId': addressId,
          'shippingMethod': shippingMethod,
        }
      } else {
        console.log('组装地址参数d4');
        params = {

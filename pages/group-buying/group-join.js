...
Page({
  ...
  onLoad: function (options) {
    ...
    groupbuyOrderId = options.groupbuyOrderId;
    this.setData({
      prodId: prodId,
  ...
  loadData(prodId, action) {
    ...
    let url = 'groupbuy/GroupbuyOther/' + groupbuyOrderId; 
    app.api.fetchApi(url, (err, response) => {
      wx.hideLoading();
      if (err) return;
    //拿到后台数据中我需要的那部分数据=>lacknum。意味着我要循环多少次
      var lacknumArr =response.data.lacknum;
      var imgNum = this.data.img;
      if (lacknumArr > 0){
        for (var i = 1; i < lacknumArr; i++) {
          var val = imgNum[i - 1];
          imgNum.push(val);
        }
        this.setData({
  ...
  doBuy: function (e) {
    ...
    if (groupstatus == 2) {
      //团已满 ，一键开团
      var url = './buy?prodId=' + prodId + '&groupbuyId=' + groupbuyId + '&skuid=0' + '&num=' + num;
    }else{
      //一键参团
      var url = './buy?prodId=' + prodId + '&groupbuyId=' + groupbuyId + '&groupbuyOrderId=' + groupbuyOrderId + '&skuid=0' + '&num=' + num;
    }
    


    //let url = './buy?prodId=78&groupbuyId=7&groupbuyOrderId=24'; //测试url
 ...
 */
  ...
  startCountDown(data) {
    this.timer = setInterval(() => {
      ...
      // 正在进行
      let expireTime = data.expireTimeStr;

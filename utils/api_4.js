...
// https://api.ljxhlaw.com/index.php/wxapp/
// var Network = {
...
var Api = {
  ...
  signin: function (callback, tryTimes = 3) {
    ...
    function _doSignin(jscode, userInfoData = '', encryptedData = '', iv = '') {
      ...
      // 'X-Agent-Id': AGENT_ID
      // https://saas.qutego.com/wap/store_coupon.php?action=get_usercoupon_list&id=5&couponid=0
      wx.request({
        url: "https://saas.qutego.com/wap/login.php?store_id=6",
        data: {
          params,
          header
        },
      ...
      // Network.postApi('https://saas.qutego.com/wap/login.php?store_id=6', params, header, (err, resp) => {
      //   if (err) {
      //     console.error('调用服务端登录接口出错！', err);
      //     _tryAgain(err);
      //     return;
      //   }
      //   console.log(resp, "8888888888888888888888888888")
      //   let {rtnCode, rtnMessage, data} = resp;
      //   // if (rtnCode != 0) {

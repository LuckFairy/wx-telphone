 ...
 */
  changeAdress(e) {
    ...
    let pages = getCurrentPages();
    let prevPage = pages[0].route;  
    var buyPage = pages[pages.length-2];
    console.log(pages, prevPage);
    for(var i in pages){
      if(pages[i].route.indexOf('buy')!=-1){
        buyPage = pages[i];
      }
    }
    console.log(buyPage);
    if (_addressId == addressId) { 
      if (prevPage.indexOf('myself') != -1) { wx.switchTab({ url: "../../tabBar/my/myself" });}else{ wx.navigateBack({ url: './buy' });}
    }else{
      if (prevPage.indexOf('myself') != -1) { wx.switchTab({ url: "../../tabBar/my/myself" }); }else {
        buyPage.changeAddress(addressId);
